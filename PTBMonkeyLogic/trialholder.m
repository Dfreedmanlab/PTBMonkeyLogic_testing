...
hotkey('6', 'simulation_positions(1,1,1);');
hotkey('4', 'simulation_positions(1,1,-1);');
hotkey('8', 'simulation_positions(1,2,1);');
hotkey('2', 'simulation_positions(1,2,-1);');

hotkey('right', 'simulation_positions(1,3,1);');
hotkey('left', 'simulation_positions(1,3,-1);');
hotkey('up', 'simulation_positions(1,4,1);');
hotkey('down', 'simulation_positions(1,4,-1);');
...
if stimuli == -1, %initialize
    ...
    DAQ = varargin{3};
    mlvideo('setbg', ScreenData.dptr, ScreenData.BackgroundColor);
    mlvideo('flip', ScreenData.dptr);
...
if ScreenData.PhotoDiode > 1 && videochange && ~update_cursor,
    if ~ScreenData.PdStatus,
        mlvideo('drawtex', ScreenData.dptr, ScreenData.PdBuffer, ScreenData.PdX, ScreenData.PdY, ScreenData.PdXsize, ScreenData.PdYsize);
        ScreenData.PdStatus = 1;
    else
        mlvideo('drawtex', ScreenData.dptr, ScreenData.PdBufferBlack, ScreenData.PdX, ScreenData.PdY, ScreenData.PdXsize, ScreenData.PdYsize);
        ScreenData.PdStatus = 0;
    end
end
...
if update_cursor,
    mlvideo('drawtex', ScreenData.dptr, ScreenData.CursorBuffer, cursorpos(1), cursorpos(2), ScreenData.CursorXsize, ScreenData.CursorYsize);
...
if ~isempty(behavioralcode), %syncs the code with the screen flip
    mlvideo('waitflip', ScreenData.dptr, yrasterthresh);
    [tflip framenumber] = trialtime;
    while ~mlvideo('verticalblank', ScreenData.dptr)
	end
...
if (t1 - last_jtrace_update) > ScreenData.UpdateInterval,
    ...
    if ScreenData.ShowCursor,
        if ~isnan(cxpos_last),
            mlvideo('drawtex', ScreenData.dptr, ScreenData.CursorBlankBuffer, cxpos_last, cypos_last, ScreenData.CursorXsize, ScreenData.CursorYsize);
        end
        cxpos = floor((ScreenData.Xsize/2) + (ScreenData.PixelsPerDegree*xp));
        cypos = floor((ScreenData.Ysize/2) - (ScreenData.PixelsPerDegree*yp));
        mlvideo('drawtex', ScreenData.dptr, ScreenData.CursorBuffer, cxpos, cypos, ScreenData.CursorXsize, ScreenData.CursorYsize);
        mlvideo('flip', ScreenData.dptr);
        cxpos_last = cxpos;
        cypos_last = cypos;
    end
...
if ~isempty(varargin),
    ...
    mlvideo('setbg', ScreenData.dptr, color);
    mlvideo('flip', ScreenData.dptr);
end
...
if ~isempty(varargin),
    ...
    if var == -1,
        ...
        k = 1000;
        ScreenData = varargin{2};
        while mlvideo('verticalblank', ScreenData.dptr), end
        while ~mlvideo('verticalblank', ScreenData.dptr), end
...
persistent keynumbers keycallbacks
...
try
	keynum = KbName(keyval);
catch %#ok<CTCH>
	error('Hotkey must be a valid single keyboard key.');
end
