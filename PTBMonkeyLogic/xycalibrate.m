...
elseif ismember(gcbo, get(fig, 'children')),
    ...
    switch callertag,
        ...
        case 'startcal',
            ...
            [yis xis ~] = size(fixspot);
			FixBuffer = mlvideo('maketex', deviceptr, fixspot);
            FixXsize = xis;
            FixYsize = yis;
            ...
            while quitflag == 0,
                ...
                mlvideo('setbg', deviceptr, ScreenInfo.BackgroundColor);
                mlvideo('drawtex', deviceptr, FixBuffer, xp, yp, FixXsize, FixYsize);
                mlvideo('flip', deviceptr);
                dotison = 1;
                ...
                while t2 < maxduration,
                    ...
                    if ~isempty(kb),
                        ...
                        if kb == KbName('space') && dotison, %space: process target
                        ...
                        elseif kb == KbName('p'), %p for previous
                            t2 = maxduration;
                            i = i - 1;
                            if i < 1,
                                i = numtargets;
                            end
                        elseif kb == KbName('n'), %n for next
                            t2 = maxduration;
                            i = i + 1;
                            if i > numtargets,
                                i = 1;
                            end
                        elseif kb == KbName('q') || kb == KbName('esc'), %esc or q for quit
