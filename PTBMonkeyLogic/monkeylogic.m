...
fprintf('<<< MonkeyLogic >>> Starting up video...\n');
...
fprintf('<<< MonkeyLogic >>> Successfully initialized video.\n');

% per MS: calculate video frame length for use with movies...
fprintf('<<< MonkeyLogic >>> Calculating video frame length...\n');
numframes = 10;
flength = zeros(numframes, 2);
k = 1000;
while mlvideo('verticalblank', ScreenInfo.dptr), end
tic;
for i = 1:numframes,
    while ~mlvideo('verticalblank', ScreenInfo.dptr), end
    flength(i, 1) = k*toc;
...
for i = 1:ScreenInfo.BufferPages,
    mlvideo('setbg', ScreenInfo.dptr, [0 0 0]);
    mlvideo('flip', ScreenInfo.dptr);
end
close_video(ScreenInfo);
...
if pre,
    ...
    if exist(file, 'file'),
        ...
        for fnum = 1:numframes,
            imdata = M(:, :, :, fnum);
			vbuf = mlvideo('maketex', ScreenInfo.dptr, imdata);
            vbufnum = vbufnum + 1;
        ...
        if exist(file, 'file'),
            ...
            while exist(file, 'file');
            	load(file); %gets M, xis, yis
                thisnumframes = size(M, 4);
...
if ~preprocessed,
    ...
    [yis xis dim3] = size(mov(:, :, :, 1));
    
    M = zeros(yis, xis, dim3, numframes);
	for fnum = 1:numframes,
        imdata = mov(:, :, :, fnum);
        M(:, :, :, fnum) = imdata;
	end
...
for obnum = 1:lc, %first check for user-generated images
    ...
    if strcmp(C(obnum).Type, 'gen')
		...
		elseif ndims(imdata) == 2,
            ...
            fprintf('*** WARNING *** image data for "gen" object created by %s contains an unexpected number of dimensions\n', fname);
...
Priority(0);
mlkbd('release');
mlvideo('showcursor', ScreenInfo.Device);
close_video(ScreenInfo);
close_daq(DaqInfo);
fclose(fidbhv);
...
try
    mlvideo('init');
    deviceptr = mlvideo('setmode', ScreenInfo.Device, ScreenInfo.BufferPages);
    pause(1);
    mlvideo('setbg', deviceptr, ScreenInfo.BackgroundColor);
    mlvideo('flip', deviceptr);
    mlvideo('hidecursor', ScreenInfo.Device);

catch ME
    fprintf('Video Initialization Error\n%s\n',getReport(ME));
    mlvideo('showcursor', ScreenInfo.Device);
    mlvideo('closewin', deviceptr);
    mlvideo('release');
    return
end
...
if ScreenInfo.PhotoDiode > 1,
    ...
    img = ones(xis, yis, 3);
	ScreenInfo.PdBuffer = mlvideo('maketex', deviceptr, img);
    img = zeros(xis, yis, 3);
	ScreenInfo.PdBufferBlack = mlvideo('maketex', deviceptr, img);
...
[yis xis ~] = size(imdata);
ScreenInfo.CursorBuffer = mlvideo('maketex', deviceptr, imdata);
...
ScreenInfo.CursorBlankBuffer = mlvideo('maketex', deviceptr, blankbuffer);
ScreenInfo.CursorXsize = xis;
ScreenInfo.CursorYsize = yis;
ScreenInfo.dptr = deviceptr;		%Store device pointer. This might make mlvideo routines a bit faster.
...
if ScreenInfo.PhotoDiode > 1,
    mlvideo('closewin', ScreenInfo.PdBuffer);
end
mlvideo('closewin', ScreenInfo.CursorBuffer);
mlvideo('closewin', ScreenInfo.CursorBlankBuffer);
mlvideo('showcursor', ScreenInfo.Device);
mlvideo('closewin', ScreenInfo.dptr);
mlvideo('release');
ScreenInfo.IsActive = 0;
ScreenInfo.dptr = [];
...
if ~isempty(kb) || remotecommand,
    ...
    if kb == KbName('r'), % "r" for reward
        goodmonkey(100);
    elseif kb == KbName('p') || kb == KbName('esc') || remotecommand, % "p" or esc for pause
        ...
        t1 = toc;
        while resumeflag == 0,
            if isempty(kb) || (~(remotecommand && kb == KbName('q')) && ~(remotecommand && kb == KbName('space'))),
                kb = mlkbd('getkey');
            end
            if ~isempty(kb) && kb == KbName('space'),
                % spacebar to resume
                resumeflag = 1;
                delete([texth texth0 texth1 texth2 texth3 texth4 texth5 texth6 texth7]);
                drawnow;
            elseif ~isempty(kb) && kb == KbName('e') && EyeSignalInUse,
            ...
            elseif ~isempty(kb) && kb == KbName('j') && JoystickInUse,
            ...
            elseif ~isempty(kb) && kb == KbName('s'),
                ...
                drawnow;
            elseif ~isempty(kb) && kb == KbName('d') && EyeSignalInUse,
