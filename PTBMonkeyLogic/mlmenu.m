...
if isempty(mlf),
    ...
    disp(' ')
    disp(' ')
    fprintf('<<< MonkeyLogic >>> Revision Date: %s...\n', lastupdate)
    ...
    if ~isempty(envOS),
        fprintf('<<< MonkeyLogic >>> Operating System: %s...\n', envOS)
    end
    if usejava('jvm'),
        disp('<<< MonkeyLogic >>> *** JAVA Virtual Machine is Running ***');
    end
    if ~isempty(envCN),
        fprintf('<<< MonkeyLogic >>> Computer Name: %s...\n', envCN)
    end
    if ~isempty(envUSER),
        fprintf('<<< MonkeyLogic >>> Logged in as "%s"...\n', envUSER)
    else
    ...
    if ~isempty(envNOP),
        ...
        if envNOP > 1,
            fprintf('<<< MonkeyLogic >>> Detected %i "%s" processors...\n', envNOP, envPRC)
        else
            fprintf('<<< MonkeyLogic >>> Detected only %i "%s" processor...\n', envNOP, envPRC)
        end
    end
    envMVER = version;
    fprintf('<<< MonkeyLogic >>> Matlab version: %s...\n', envMVER)
    numloops = 1000;
    t = zeros(numloops, 1);
    tic;
    for i = 1:numloops,
...
elseif ismember(gcbo, get(findobj('tag', 'monkeylogicmainmenu'), 'children')) || reloadcondfile || ismember(get(gcbo, 'parent'), get(findobj('tag', 'monkeylogicmainmenu'), 'children')),
    ...
    switch callertag,
        ...
        case 'videotest',
            ...
            refreshrate = validrefresh(get(findobj(gcf, 'tag', 'refreshrate'), 'value')); %#ok<NASGU>
            ...
            try
                mlvideo('init');
                deviceptr = mlvideo('setmode', videodevice, bufferpages);
                tex = zeros(numcycles, 1);
                ...
                for i = 1:numcycles,
					...
					tex(i) = mlvideo('maketex', deviceptr, testpattern);
                end
                mlmessage('Displaying test pattern...');
                t1 = tic;
                for j = 1:numcycles,
                    for i = 1:numcycles,
                        mlvideo('drawtex', deviceptr, tex(i));
                ...
                t2 = toc(t1);
                for i = 1:numcycles,
                    mlvideo('closewin', tex(i));
                end
                mlvideo('showcursor', videodevice);
        ...
        case 'viewplay',
            ...
            if strcmpi(t, 'fix') || strcmpi(t, 'pic') || strcmpi(t, 'dot') || strcmpi(t, 'crc') || strcmpi(t, 'sqr') || strcmpi(t, 'gen'),
                ...
                try
                    mlvideo('init');
                    deviceptr = mlvideo('setmode', vdev, bufferpages);
                    mlvideo('setbg', deviceptr, bgcolor);
                    mlvideo('flip', deviceptr);
                    mlvideo('hidecursor', vdev);
					texptr = mlvideo('maketex', deviceptr, imdata);
					mlvideo('drawtex', deviceptr, texptr, xscreenpos, yscreenpos, xis, yis);
                    mlvideo('flip', deviceptr);
            ...
            elseif strcmpi(t, 'mov'), %movie
                ...
                try
                    ...
                    for fnum = 1:numframes,
						...
						texptr = mlvideo('maketex', deviceptr, imdata);
                        vbufnum = vbufnum + 1;
                        vbuffer(vbufnum) = texptr; %#ok<AGROW>
                    end
                    lastbuffer = vbufnum;
                    ...
                    while ~keypress,
                        if ~keypress,
                            for vbufnum = 1:lastbuffer,
								mlvideo('drawtex', deviceptr, vbuffer(vbufnum), xscreenpos, yscreenpos, xis, yis);
                                mlvideo('flip', deviceptr);
                                kb = mlkbd('getkey');
                                if ~isempty(kb),
                                    keypress = 1;
                    ...
                    if strcmpi(typ, 'mov'),
                        ...
                        if maxframes >= numframes,
                            ...
                            for j = 1:(length(partition)-1),
                                ...
                                [yis xis dim3] = size(mov(:,:,:,1));
                                M = zeros(yis, xis, dim3, thisnumframes);
